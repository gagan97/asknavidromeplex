        ...
        if library_key:
            ...
            for track in hub_section_results:
                ...
                if track_id and track_id not in seen_ids: 
                    ...
                    all_results.append(track)
            self.logger.debug(f'Hub search with section found {len(hub_section_results)} tracks ({len(hub_section_results) - len([t for t in hub_section_results if t.get("id") in seen_ids])} new)')
