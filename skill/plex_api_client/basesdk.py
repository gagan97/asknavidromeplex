...
class BaseSDK:
    ...
    def _build_request_with_client(
        ...
        if url is None:
            ...
            # Special handling for 'query' parameter in search requests
            # This ensures spaces are encoded as %20 instead of + in search queries
            if 'query' in query_params and '/hubs/search' in path:
                # Make a copy to avoid mutating the original
                import urllib.parse
                query_value = query_params['query']
                # Use a custom encoding approach that preserves %20 for spaces
                if isinstance(query_value, str):
                    # Replace spaces with %20 and then apply standard encoding for other characters
                    encoded_query = query_value.replace(' ', '%20')
                    # Ensure proper encoding of other special characters while preserving %20
                    encoded_query = urllib.parse.quote(encoded_query, safe='%')
                    query_params['query'] = encoded_query
            
            # Fix for numeric parameters - ensure they are sent as proper integers without decimal points
            # These need to be integers without decimal points in the query string
            if '/hubs/search' in path:
                # Convert all numeric parameters to proper integers without decimal points
                # This ensures numeric parameters are sent as "3" and not "3.0"
                for param_name, param_value in list(query_params.items()):
                    if param_value is not None:
                        try:
                            # Check if it's a numeric value that should be an integer
                            if isinstance(param_value, (int, float)):
                                # Convert floats to integers and store as strings to ensure no decimal point
                                query_params[param_name] = str(int(param_value))
                            elif isinstance(param_value, str) and param_value.replace('.', '', 1).isdigit():
                                # If it's a string that represents a number, convert to integer
                                query_params[param_name] = str(int(float(param_value)))
